    ...
    <div className="h-screen w-screen overflow-hidden" style={{ background: '#0f1117' }}>
      ...
      <div
        ...
        style={{
          ...
          opacity: state.viewMode === 'map' ? 1 : 0,
          pointerEvents: state.viewMode === 'map' ? 'auto' : 'none',
        }}
      >
        <StrataMap />
      </div>
        ...
        {state.isLoading && (
          ...
          >
            <div className="relative flex flex-col items-center">
              {/* Spinner */}
              <div className="w-16 h-16 border-4 border-shallows/20 border-t-gold rounded-full animate-spin mb-6" />
              
              <motion.h2 
                initial={{ y: 20, opacity: 0 }}
                animate={{ y: 0, opacity: 1 }}
                className="text-2xl font-bold text-shallows tracking-wider"
              >
                STRATA<span className="text-gold">DESK</span>
              </motion.h2>
              <motion.p 
                initial={{ y: 20, opacity: 0 }}
                animate={{ y: 0, opacity: 1 }}
                transition={{ delay: 0.1 }}
                className="text-shallows/60 mt-2 text-sm uppercase tracking-[0.2em]"
              >
                Initializing Subsurface Data
              </motion.p>
            </div>
          </motion.div>
        )}
