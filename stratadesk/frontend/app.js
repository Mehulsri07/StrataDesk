...
// ── API base URL — works both locally (file://) and via nginx proxy ──────────
const API_BASE = '/api';
...
async function loadData() {
    try {
        state.borewells = await apiFetch('/borewells');
    } catch (e) {
...
// saveData is no longer needed — every mutation calls the API directly.
// Kept as a no-op so any stray call doesn't throw.
function saveData() {}
...
function createPopupContent(borewell) {
    const timeAgo = Math.round((new Date() - new Date(borewell.updatedAt)) / 60000);
    ...
    return `
        <div style="min-width: 140px; font-size: 0.875rem;">
            ...
            <p style="font-size: 0.65rem; opacity: 0.5; margin-top: 0.25rem;">Edited ${timeStr}</p>
...
function toggleCrossSection(id) {
    ...
    if (!borewell) return;
    const next = !borewell.selectedForCrossSection;
    apiFetch(`/borewells/${id}`, {
        method: 'PATCH',
        body: JSON.stringify({ selectedForCrossSection: next }),
    }).then(updated => {
        Object.assign(borewell, updated);
        updateCrossSection();
...
function updateForm() {
    ...
    document.getElementById('waterLevel').value = borewell?.waterLevel || '';
...
function saveBorewell() {
    ...
    if (!name) { showToast('Borewell name is required', 'error'); return; }
    if (totalDepth <= 0) { showToast('Total depth must be greater than 0', 'error'); return; }
    ...
    const data = {
        ...
        totalDepth,
        waterLevel: parseFloat(document.getElementById('waterLevel').value) || null,
    ...
    if (existing) {
        // PUT — full update, preserve layers
        const payload = { ...existing, ...data, layers: existing.layers };
        apiFetch(`/borewells/${existing.id}`, {
            method: 'PUT',
        ...
        apiFetch('/borewells', {
            method: 'POST',
            body: JSON.stringify(newBorewell),
        }).then(created => {
            state.borewells.push(created);
            state.activeBorewellId = created.id;
            state.pendingLatLng = null;
            showToast('Borewell saved', 'success');
            updateUI();
        }).catch(e => showToast(e.message, 'error'));
...
function deleteBorewell() {
    ...
    if (!confirm('Delete this borewell?')) return;

    const id = state.activeBorewellId;
    apiFetch(`/borewells/${id}`, { method: 'DELETE' })
        .then(() => {
            state.borewells = state.borewells.filter(b => b.id !== id);
            state.activeBorewellId = null;
...
// ── Shared helper: persist the active borewell's current state to the API ────
function persistBorewell(borewell) {
    return apiFetch(`/borewells/${borewell.id}`, {
        method: 'PUT',
...
function editDepth(layerId, field, currentValue) {
    ...
    layer[field] = value;
    borewell.layers.sort((a, b) => a.startDepth - b.startDepth);
    if (checkOverlap(borewell.layers)) showToast('Layer depth overlap detected!', 'error');
    persistBorewell(borewell);
}
...
function autoCloseDepth() {
    ...
    persistBorewell(borewell).then(() => showToast('Last layer closed to total depth', 'success'));
}
...
function renderStrataChart() {
    ...
    if (borewell.waterLevel && borewell.waterLevel > 0 && borewell.waterLevel <= borewell.totalDepth) {
        const wy = (borewell.waterLevel / borewell.totalDepth) * height;
        const wlLine = document.createElementNS('http://www.w3.org/2000/svg', 'line');
        wlLine.setAttribute('x1', barX - 10);
        wlLine.setAttribute('y1', wy);
        wlLine.setAttribute('x2', barX + barWidth + 10);
        wlLine.setAttribute('y2', wy);
        wlLine.setAttribute('stroke', '#3b82f6');
        wlLine.setAttribute('stroke-width', '1.5');
        wlLine.setAttribute('stroke-dasharray', '4 2');
        svg.appendChild(wlLine);

        const wlText = document.createElementNS('http://www.w3.org/2000/svg', 'text');
        wlText.setAttribute('x', barX + barWidth + 12);
        wlText.setAttribute('y', wy - 3);
        wlText.setAttribute('fill', '#3b82f6');
        wlText.setAttribute('font-size', '8');
        wlText.setAttribute('font-weight', 'bold');
        wlText.textContent = `Water Table: ${borewell.waterLevel} ft`;
        svg.appendChild(wlText);
    }
    
    // Compute percentages for stats header
    const matSums = {};
    borewell.layers.forEach(l => {
        matSums[l.material] = (matSums[l.material] || 0) + (l.endDepth - l.startDepth);
    });
    const statsHtml = Object.entries(matSums)
        .sort((a,b) => b[1] - a[1])
        .slice(0, 3)
        .map(([mat, sum]) => `${mat}: ${Math.round((sum/borewell.totalDepth)*100)}%`)
        .join(' | ');
    if (statsContainer) statsContainer.innerHTML = statsHtml ? `Composition: ${statsHtml}` : '';
...
document.getElementById('exportPNG')?.addEventListener('click', async () => {
    ...
    const borewell = getActiveBorewell();
    if (!chartContainer.querySelector('svg') || !borewell) return;

    try {
        const pTitle = borewell.name || "Borewell";
        ...
        link.download = `StrataChart_${pTitle.replace(/\s+/g, '_')}.png`;
        link.href = canvas.toDataURL('image/png');
        link.click();
        
        showToast('PNG successfully exported!', 'success');
    } catch (error) {
